...
def build_sentenance_start_end(doc, words_ts, audio_buffer):
    ...
    for sent in doc.sents:
        ...
        try:
            ...
            log.error(f"mismatched word - {word_idx=} {end_word_idx=} {sent_words[0]=} { words_ts[word_idx][0]=} ")
            raise

        start = words_ts[word_idx][1] - audio_buffer if words_ts[word_idx][1] > 0  else words_ts[word_idx][1]
...
def translate_sentenaces(sents, from_code, to_code):
    return [
        argostranslate.translate.translate(s, from_code, to_code)
        for s in sents
    ]
...
def main(
    ...
    created_files = split_video_into_audio_seg(source_file, _ts, base_name, audio_save_dir)
...
if __name__ == '__main__':
    ...
    parser.add_argument("source_file", help="The full path to the video to process", type=Path)
    parser.add_argument(
        "--input-language", '-i', help=f"Language code for input language ({'.'.join(SPACY_MODELS.keys())})", default='es', type=str)
    parser.add_argument(
        "--output-language", '-o', help=f"Language code for output language ({'.'.join(SPACY_MODELS.keys())})", default='en', type=str)
    parser.add_argument(
        "--audio-save-dir", '-s', help="Directory to save audio clips in", default='./out', type=Path)
    parser.add_argument(
        "--json-transcribe-file", '-j', help="Path to load whisper's json transcription file, will skip doing a new transcription", type=Path)
    parser.add_argument(
        "--audio-buffer", '-b', help="Time to add on to start and end of audio sentenance clips", default=AUDIO_BUFFER, type=float)
    parser.add_argument(
        "--save_json_file", '-w',
